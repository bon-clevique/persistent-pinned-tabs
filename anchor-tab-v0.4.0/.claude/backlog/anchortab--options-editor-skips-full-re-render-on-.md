...
# AnchorTab: options editor skips full re-render on text input changes

## Goal
Preserve focus and scroll position while typing URL / name in options editor

## Context
Arch review #4. document.body.innerHTML reset on every change event loses focus. Skip render() for pure text patches; only re-render on structural changes.
